  ...
  return h('div',{className:'pg today-page'},
    ...
    h('div',{className:'dcard'},
      ...
      h('div',{className:'dacts'},
        h('button',{className:'dlisten',onClick:()=>{if(typeof window==='undefined'||!window.speechSynthesis)return;window.speechSynthesis.cancel();const voices=window.speechSynthesis.getVoices();const uttSk=new SpeechSynthesisUtterance(v.sanskrit);uttSk.lang='hi-IN';uttSk.rate=0.65;uttSk.pitch=0.9;const hiVoice=voices.find(vv=>vv.lang==='hi-IN')||voices.find(vv=>vv.lang.startsWith('hi'));if(hiVoice)uttSk.voice=hiVoice;const uttPause=new SpeechSynthesisUtterance('...');uttPause.rate=0.1;uttPause.volume=0;const uttEn=new SpeechSynthesisUtterance(v.meaning.replace(/[\u201c\u201d\u0022]/g,''));uttEn.lang='en-GB';uttEn.rate=0.78;uttEn.pitch=0.88;uttEn.volume=1;const enVoice=voices.find(vv=>vv.lang==='en-GB'&&vv.name.toLowerCase().includes('female'))||voices.find(vv=>vv.lang==='en-GB')||voices.find(vv=>vv.lang.startsWith('en'));if(enVoice)uttEn.voice=enVoice;const uttRefl=new SpeechSynthesisUtterance(v.reflection);uttRefl.lang='en-GB';uttRefl.rate=0.75;uttRefl.pitch=0.85;uttRefl.volume=1;if(enVoice)uttRefl.voice=enVoice;window.speechSynthesis.speak(uttSk);window.speechSynthesis.speak(uttPause);window.speechSynthesis.speak(uttEn);window.speechSynthesis.speak(uttPause);window.speechSynthesis.speak(uttRefl);}},h(SvgPlay),' Listen'),
